        ...
        showMessage: function(objeto){
            GetHTML('./messageBox/index.html', 'get',  function(obj) {
                ...
                if(!!objeto.boxInput){
                    ...
                    !objeto.boxInput.boxCheck ? 
                        messageBox.querySelector('.boxInput').querySelector('.boxcheck').remove() : 
                        messageBox.querySelector('.boxInput').querySelector('.boxCheck').innerHTML = objeto.boxInput.boxCheck.corpo
                    ...
                    if(!!objeto.boxInput){
                        messageBox.querySelector('.boxButtons').querySelector('.accept').addEventListener('click',()=>{
                            if (messageBox.querySelector('.boxInput').querySelector('textarea').value.length > 0){
                                ...
                                if(!!objeto.boxInput.boxCheck){
                                    BoxMessage.objeto.check = !!messageBox.querySelector('input[name="token"]:checked') ?
                                    messageBox.querySelector('input[name="token"]:checked').value : null
                                }
